...
public class DefaultStudentService implements StudentService {
    ...
    public Collection<Student> getPassedStudents() {
        ...
        return studentRepository.findAll().stream().filter(student -> Objects.nonNull(student.getScore())).filter(student -> !student.getScore().isFail()).collect(
            Collectors.toList());
    }
    ...
    public Collection<Student> getStudentsOrderByScore() {
        ...
        return studentRepository.findAll().stream().filter(student -> Objects.nonNull(student.getScore())).sorted(
            (o1, o2) -> o2.getScore().getScore()-o1.getScore().getScore()).collect(Collectors.toList());
    }
